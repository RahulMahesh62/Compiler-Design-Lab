...
int isop(char *buff){
char aops[][2]={"+","-","*","/","%"}, //Arithmetic operators
relops[][3]={"==","!=","<=",">=",">","<"},logops[][3]={"&&","||","!"}, //Relational operators
asgops[][3]={"+=","-=","/=","*=","=","%="}; 
int i;
for(i=0;i<6;i++){
...
void main(){
	FILE *fp=fopen("code.c","r"); //File pointer to open source file in read mode
	...
	while(tok != NULL){
		if(iskeyword(tok)){
			...
			char string[1024];
		strcpy(string,tok);if(!endsWith("\"",tok)){ 
		do{
			tok = strtok(0," ");
			if(tok != NULL){
			strcat(string," ");
		...
		printf("Total tokens : %d\n",count);//Displays total number of tokens in src pgm
